...
function expert_hotel_booking_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'expert-hotel-booking' ) );
		if ( $categories_list && expert_hotel_booking_categorized_blog() ) {
			/* translators: %1$s: Posted in. */
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'expert-hotel-booking' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'expert-hotel-booking' ) );
		if ( $tags_list ) {
			/* translators: %1$s: Tagged. */
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'expert-hotel-booking' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
...
function expert_hotel_booking_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'expert_hotel_booking_categories' ) ) ) {
		...
		set_transient( 'expert_hotel_booking_categories', $all_the_cool_cats );
	}
